#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# 41_methylation_overview.py — Four-panel overview of per-copy CpG methylation across the 5S array (ONT).
...
Input : 5S_rDNA.db (tables copy_methylation, copy, haplotype, assembly).
Output: <FIVES_OUT>/03_methylation_full215/00_methylation_overview.pdf
...
# detect ALU columns
has_alu = con.execute(
    "SELECT COUNT(*) FROM pragma_table_info('copy_methylation') "
    "WHERE name='alu_n'"
).fetchone()[0] > 0
alu_populated = has_alu and con.execute(
    "SELECT SUM(alu_n) FROM copy_methylation").fetchone()[0] > 0
...
df = pd.read_sql("""
    SELECT cm.copy_id, cm.n_conf_calls, cm.mean_meth,
           ...
           c.unit_start_local, h.hap_label, h.haplotype_id,
...
df["pct_pos"] = (df.groupby("haplotype_id")["unit_start_local"]
                   .rank(method="first").sub(1)
                 / df.groupby("haplotype_id")["unit_start_local"]
...
plt.tight_layout()
out = OUTDIR / "00_methylation_overview.pdf"
