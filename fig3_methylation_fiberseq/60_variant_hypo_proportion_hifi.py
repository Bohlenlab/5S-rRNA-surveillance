#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# 60_variant_hypo_proportion_hifi.py — Methylation distributions and proportion-low bars binned by variant count for the 5S gene, ALU, and NTS-post regions (HiFi).
...
# -----------------------------------------------------------------------------
"""
60_variant_hypo_proportion_hifi.py
...
Output: <FIVES_OUT>/03_methylation_full215/24b_variant_hypo_proportion_hifi.pdf
...
# ── region definitions ────────────────────────────────────────────────────────
# ALU bins: 0 / 1 / 2 / ≥3 ;  NTS-post: 0 / 1 / 2-3 / 4-5 / ≥6
SHARED_BINS   = [-1, 0, 1, 2, 200]
SHARED_LABELS = ["0", "1", "2", "≥3"]
# HiFi 5S gene: only 15 copies carry ≥3 gene variants (vs 417 with 2) — too few for a
# stable bar/Wilson CI, so the gene panel merges 2 and ≥3 into one ≥2 bin.
GENE_BINS     = [-1, 0, 1, 200]
GENE_LABELS   = ["0", "1", "≥2"]

REGIONS = [
    ("5S gene (119 bp)",   "n_snv_5s_gene",  "meth_pct",
     GENE_BINS, GENE_LABELS, "#2166AC"),
...
out = OUTDIR / "24b_variant_hypo_proportion_hifi.pdf"
...
# gene: 0/1/≥2 → meth_pct (HiFi: ≥3 merged into ≥2, too few copies)
gene_dict = {lb: df.loc[pd.cut(df["n_snv_5s_gene"],
             bins=GENE_BINS, labels=GENE_LABELS) == lb, "meth_pct"]
             for lb in GENE_LABELS}
make_wide(gene_dict, "n_snv_5s_gene").to_csv(
    DDIR / "24b_gene_variants_meth_wide_hifi.tsv", sep="\t", index=False)
...
make_wide(alu_dict, "n_snv_alu").to_csv(
    DDIR / "24b_alu_variants_meth_wide_hifi.tsv", sep="\t", index=False)
...
make_wide(nts_dict, "n_snv_other_nts").to_csv(
    DDIR / "24b_nts_variants_meth_wide_hifi.tsv", sep="\t", index=False)
