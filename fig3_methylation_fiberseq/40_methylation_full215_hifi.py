#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# 40_methylation_full215_hifi.py — Per-copy CpG methylation figures across the 5S array for the full 215-proband cohort (HiFi), from the SQLite database.
...
# -----------------------------------------------------------------------------
"""
40_methylation_full215_hifi.py

Per-copy CpG methylation figures across the 5S array for the full 215-proband
cohort (HPRC Year 1 + Release 2), from the SQLite database (HiFi).

Data source: 5S_rDNA.db  ·  copy_methylation_hifi table
...
Array position (pct_pos) is computed from copy rank in ascending copy_number
order per haplotype (cross-contig order); no orientation flip is applied.

Input : 5S_rDNA.db (tables copy_methylation_hifi, copy, variant, haplotype, assembly).
Output: <FIVES_OUT>/03_methylation_full215/{figname}_hifi.pdf
Data:   <FIVES_OUT>/03_methylation_full215/data/{figname}_hifi.tsv  (wide format)
...
df = pd.read_sql_query(f"""
    SELECT
        ...
        c.unit_start_local, c.copy_number,
        c.n_snv_gene, c.n_snv_5s_gene, c.n_snv_nts_pre, c.n_snv_nts_post,
        h.hap_label, h.haplotype_id, h.array_order_resolved,
...
# unit_start_local is contig-local). Partial-order haplotypes excluded.
df.loc[df["array_order_resolved"] == "partial", "copy_number"] = np.nan
def pct_positions(group):
    ranked = group["copy_number"].rank(method="first") - 1
    n = len(ranked)
...
fig.suptitle(f"5S rRNA CpG methylation — {cohort_title()}\n"
             "Regional summaries (NTS-pre / gene / NTS excl. ALU) from SQLite copy_methylation_hifi table",
             fontsize=9, y=1.01)
...
# ── try to build a truly positional heatmap from copy_meth_pos_hifi ────────────────
con = sqlite3.connect(DB_PATH)
_has_pos_table = con.execute(
    "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='copy_meth_pos_hifi'"
).fetchone()[0] > 0
...
if _has_pos_table:
    print("  copy_meth_pos_hifi table found — building positional heatmap …")
    ...
    pos_data = pd.read_sql_query("""
        ...
        FROM copy_meth_pos_hifi p
        JOIN copy_methylation_hifi cm USING(copy_id)
    ...
    fig_png.savefig(OUTDIR / "30_methylation_heatmap_hifi.png", dpi=200, bbox_inches="tight")
    plt.close(fig_png)
    # 30b: imputed, nearest
    ...
    fig_imp_png.savefig(OUTDIR / "30b_methylation_heatmap_imputed_hifi.png",
    ...
    # ── fallback: 3-region heatmap (no copy_meth_pos_hifi table yet) ───────────────
    print("  copy_meth_pos_hifi not found — falling back to 3-region heatmap.")
